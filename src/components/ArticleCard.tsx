...
const ArticleCard = ({
      ...
      <Link to={articleUrl} className="group block">
        <div className="relative overflow-hidden rounded-2xl aspect-[16/9] mb-4">
          ...
          <div className="absolute bottom-5 left-5 right-5">
            <span className="inline-block text-[11px] font-semibold uppercase tracking-wider text-white/80 mb-1.5">{category}</span>
            <h3 className="font-heading text-xl sm:text-2xl font-bold text-white leading-tight line-clamp-2">{title}</h3>
          </div>
        </div>
      </Link>
    );
  ...
  if (variant === "compact") {
    return (
      <Link to={articleUrl} className="group flex gap-4 py-4">
        ...
        <div className="flex-1 min-w-0">
          <span className="text-[11px] font-semibold uppercase tracking-wider text-primary">{category}</span>
          ...
          <span className="text-[11px] text-muted-foreground mt-1 block">{readTime} read</span>
        </div>
      </Link>
    );
  }
